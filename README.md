...
graph TD
    ...
    classDef gateway fill:#ff9,stroke:#f66,stroke-width:2px;
    ...
    subgraph Infrastructure [基础设施层]
        Nacos[Nacos 注册与配置中心]:::infra
    end

    subgraph GatewayLayer [网关层 - 核心]
        direction TB
        Ingress[统一入口]
        AuthFilter[全局认证过滤器<br/>AuthGlobalFilter]
        Sentinel[Sentinel 流量卫士<br/>限流/熔断]
        LB[LoadBalancer 负载均衡]
    end

    subgraph ServiceLayer [微服务层]
        Consumer[消费者服务<br/>Consumer Service]:::service
        Provider[提供者服务<br/>Provider Service]:::service
    end

    %% 连线关系
    User --> |HTTPS/WSS| Ingress
    Ingress --> AuthFilter
    AuthFilter --> |Token校验| Sentinel
    Sentinel --> |通过| LB
    
    LB --> |动态路由转发| Consumer
    Consumer --> |Feign RPC| Provider

    %% 注册与配置
    GatewayLayer -.-> |拉取路由/配置| Nacos
    ServiceLayer -.-> |注册/心跳| Nacos
    
    %% 描述
    note_gw[核心功能: <br/>1. 动态路由热更新<br/>2. 全链路身份透传<br/>3. 统一接口文档]
    style note_gw fill:#fff,stroke:#333,stroke-dasharray: 5 5
    GatewayLayer --- note_gw
```
